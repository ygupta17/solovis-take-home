...
## What's actually inside the FastAPI backend**

- `app/db/protocol.py` — the concurrency core. Every seat-mutating
  operation (hold, confirm, cancel, sweep, waitlist-promote) lives here and
  nowhere else, all following the same lock → recheck → mutate → commit
  shape. This is the one file to read first.
- `app/routers/` — the thin HTTP layer (`events.py`, `holds.py`,
  `waitlist.py`, `ws.py`). Turns requests into calls into `protocol.py` and
  maps its exceptions to status codes; no business logic of its own.
- `app/schemas.py` — request/response shapes and input validation
  (Pydantic), including the name/email checks on confirming a booking.
- `app/errors.py` — one exception type per domain failure (seat
  unavailable, hold expired, not your hold, etc.), so the mapping from
  "what went wrong" to "what HTTP status that means" lives in exactly one
  place instead of being guessed at in each route.
- `app/workers/sweeper.py` — the background loop that frees expired holds;
  purely a liveness/UX nicety, not something correctness depends on (see
  Concurrency above).
- `app/realtime/notify.py` — the Postgres `LISTEN`/`NOTIFY` → WebSocket
  fan-out for live seat-map updates.
- `app/config.py` / `app/dependencies.py` — environment-driven settings
  (`DATABASE_URL`, `lock_timeout`, hold TTL) and shared FastAPI
  dependencies (DB pool access, reading the session-token header).
- `app/seed.py` — the idempotent script that creates the demo events.
...
- **`seats`** — one row per physical seat: `event_id`, `section`,
  ...
  `bookings` — it's both the fast read path for the seat map and the exact
  row every mutation locks with `SELECT ... FOR UPDATE`, so there's one
  place for state to live, not two things that can drift apart. (The seat
  map query does now do one small `LEFT JOIN` to `holds` — see below — but
  that's about revealing *whose* hold a seat has, not about deriving
  status itself, which still lives only on this row.)
...
Two details that highlight the concurrency aspect of this app: transactions give up after 3 seconds waiting for a lock, so a hot
seat can't tie up connections indefinitely (confirmed under load — 500
people hitting one seat finished in ~3.67s, right at that ceiling); and
confirming a booking rechecks it's still *your specific* hold, not just
that the seat is held by someone, since a hold can quietly expire and get
handed to someone else in the meantime.
